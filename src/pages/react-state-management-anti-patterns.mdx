...
export const sections = [
  ...
  {
    title: 'When Storing is Acceptable',
    id: 'when-storing-derived-state-is-acceptable',
  },
...
State is the data your UI needs to _remember_ over time. The hardest part of state management isn't usually _how_ to store data it's deciding _what_ deserves to be state in the first place.
...
The classic shape looks like this: you have `firstName` and `lastName` in state, and then you _also_ keep `fullName` in state and try to keep it updated. Now `fullName` can disagree with the two values it's supposedly built from.
...
`visible` is just `products` + `query`. Derive it during render and the bug surface disappears. If you stored it in state instead, you'd need to update it whenever `products`, `query`, _or_ any other filter changes three update paths, three chances to forget one.
...
The rule isn't "never store derived values." It's "don't store them _by default_." There are three legitimate exceptions.
...
`useMemo` stores a _cache_ that's automatically invalidated when `points` changes. `useState` + `useEffect` stores a _copy_ that can go stale. One is an optimization; the other is an anti-pattern.
...
Sometimes you want the _initial_ value from a prop, and then let the user edit it independently. The drift is the point.
...
This is _not_ derived state `user` can't be calculated from `userId` alone, it requires an async fetch. This is exactly what state and effects are for.
...
<div
  ...
  style={{
    ...
    padding: '2.5rem 2rem',
    background:
      'linear-gradient(135deg, #fff8f5 0%, #ffffff 60%, #fff8f5 100%)',
    border: '1px solid rgba(255, 86, 0, 0.25)',
    borderRadius: 12,
    textAlign: 'center',
    boxShadow: '0 0 40px rgba(255, 86, 0, 0.08)',
  }}
>
  <p
    style={{
  ...
  >
    If you can calculate it,{' '}
    <span style={{ color: '#ff5600' }}>don't store it.</span>
  </p>
</div>
